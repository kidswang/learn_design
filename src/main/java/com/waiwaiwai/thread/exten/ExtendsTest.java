...
public class ExtendsTest {
    ...
    @Test
    public void test3(HttpSession session) {

        // 判断参数 姓名不能为空，班级不能为空

        // 查询并判断该用户是否存在

        // 一个总的积分数 查所有 分组 使用流相加、相减
        List<Person> list = new ArrayList<>();
        Map<String, List<Person>> collect = list.stream().collect(Collectors.groupingBy(Person::getGradeId));
...
//        mapList.stream().map(m -> m.entrySet().stream().sorted(Comparator.comparing()))
        ...
        public static void main (String[] args){
            ...
            List<Map<String, Object>> collect = list.stream().sorted(Comparator.comparing(ExtendsTest::comparingByAge).reversed())
